...
const MagicButton = ( {title,icon,position,handleClick,otherClasses}: {
    ...
    return (
        <button className="relative inline-flex h-12 w-full overflow-hidden rounded-lg p-[1px] focus:outline-none md:w-60 md:mt-10" onClick={handleClick}>
            ...
            <span className = {`inline-flex h-full w-full cursor-pointer items-center justify-center rounded-xl bg-slate-950 px-6 text-sm font-medium text-white backdrop-blur-3xl gap-3 ${otherClasses} `}>
                {position === 'left' && icon}
                {title}
                {position === 'right' && icon}
            </span>
